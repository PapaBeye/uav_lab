...
class Decoder:
    ...
    def __init__(self, message_definition_path):
        ...
        for xml_message in xml_messages:
            ...
            message.message_id = int(xml_message.find(Decoder.namespace + 'code').text)
            message.field_names = []
            message.field_types = []
        ...
        if packet.message_id == 125: #Message Report
            ...
            for i in range(0, len(message_def.field_names)):
                if message_def.field_types[i] == 'byte':
                    ...
                    output[message_def.field_names[i]] = struct.unpack_from('f', packet.data, offset)[0]
                    offset += 4
                elif message_def.field_types[i] == 'short':
                    output[message_def.field_names[i]] = struct.unpack_from('h', packet.data, offset)[0]
                    offset += 2
                elif message_def.field_types[i] == 'ushort':
                    output[message_def.field_names[i]] = struct.unpack_from('H', packet.data, offset)[0]
                    offset += 2
                elif message_def.field_types[i] == 'long':
                    output[message_def.field_names[i]] = struct.unpack_from('i', packet.data, offset)[0]
                    offset += 4
    ...
    def createMessagePayload(self, message_id, message_data):
        ...
        if message_id == 125: #Message Report
            ...
            for i in range(0, len(message_def.field_names)):
                if message_def.field_types[i] == 'byte':
                    payload.extend(struct.pack('B', message_data[message_def.field_names[i]]))
                elif message_def.field_types[i] == 'float':
                    payload.extend(struct.pack('f', message_data[message_def.field_names[i]]))
                elif message_def.field_types[i] == 'short':
                    payload.extend(struct.pack('h', message_data[message_def.field_names[i]]))
                elif message_def.field_types[i] == 'ushort':
                    payload.extend(struct.pack('H', message_data[message_def.field_names[i]]))
                elif message_def.field_types[i] == 'long':
                    payload.extend(struct.pack('i', message_data[message_def.field_names[i]]))
                elif message_def.field_types[i] == 'ulong':
                    payload.extend(struct.pack('I', message_data[message_def.field_names[i]]))
                else:
                    print("Unknown type: ", message_def.field_types[i])
